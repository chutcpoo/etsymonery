...
import {
  exactPdStock005R03RecoveryR02Body,
  handlePdStock005R03RecoveryR02,
  PD_STOCK_005_R03_RECOVERY_R02,
  verifyPdStock005R03RecoveryR02PostRelease,
  verifyPdStock005R03RecoveryR02ProtectedState
} from "../lib/pd-stock-005-r03-recovery-r02";
...
function core(over:Record<string,unknown>={}){
 return {
  ...
  title:PD_STOCK_005_R03_RECOVERY_R02.current.title,
  tags:[...PD_STOCK_005_R03_RECOVERY_R02.current.tags],
...
function provider(options:{persistUpload?:boolean;uploadStatus?:number}={}){
 let listing:Record<string,unknown>=core(),files=structuredClone(INITIAL),writes=0,attempts=0;
 const fetchImpl=(async(input:string|URL|Request,init?:RequestInit)=>{
  const url=String(input),method=init?.method??"GET";
  if(method==="DELETE"){
    throw new Error("DELETE_NOT_AUTHORIZED_IN_R02");
  }
  if(method==="POST"&&url.endsWith("/files")){
    attempts++;
    const form=init?.body as FormData;const name=String(form.get("name"));
    assert.equal(name,PD_STOCK_005_R03_RECOVERY_R02.target.zip.fileName);
    if(options.persistUpload!==false){
      writes++;
    ...
    return Response.json({listing_file_id:1611111111111},{status:options.uploadStatus??201});
  }
  if(method==="PATCH"){
    attempts++;writes++;
...
test("Recovery R02 identity is upload-only and protects all four existing files",()=>{
 assert.equal(PD_STOCK_005_R03_RECOVERY_R02.operationId,"PD-STOCK-005-LIVE-CATALOG-V3-R03-RECOVERY-R02-001");
 assert.equal(PD_STOCK_005_R03_RECOVERY_R02.operation,"RECOVER_UPLOAD_ZIP_THEN_PATCH_TITLE_TAGS");
 assert.equal(PD_STOCK_005_R03_RECOVERY_R02.target.zip.targetSizeBytes,62008);
 assert.equal("currentListingFileId" in PD_STOCK_005_R03_RECOVERY_R02.target.zip,false);
 const body=exactPdStock005R03RecoveryR02Body("AUTH",PD_STOCK_005_R03_RECOVERY_R02.protectedStateFingerprint);
...
test("Recovery protected-state verifier is read-only and matches partial baseline",async()=>{
 ...
 const p=provider();
 const r=await verifyPdStock005R03RecoveryR02ProtectedState({getAccessToken:async()=>"token",fetchImpl:p.fetchImpl});
...
test("Recovery R02 performs upload exact ZIP then patches title/tags, with no delete",async()=>{
 process.env.PUBLISH_WRITES_ENABLED="true";process.env.ETSY_B01_WRITE_TOKEN="w";process.env.ETSY_SHOP_ID="23582741";
 process.env.ETSY_API_KEY="k";process.env.ETSY_SHARED_SECRET="s";
 const auth="RECOVERY-R02-AUTH";
 const body=exactPdStock005R03RecoveryR02Body(auth,PD_STOCK_005_R03_RECOVERY_R02.protectedStateFingerprint);
 ...
 const r=await handlePdStock005R03RecoveryR02(body,requestHash,req,{
  repository:repo,fetchImpl:p.fetchImpl,getAccessToken:async()=>"token",
  loadAsset:async()=>Buffer.alloc(62008),verifyAsset:()=>true,now:()=>"2026-09-18T15:45:00.000Z"
 });
 const x=await r.json();
 assert.equal(r.status,200);assert.equal(x.status,"UPDATED_AND_VERIFIED");assert.equal(x.ETSY_WRITE_COUNT,2);
 assert.equal(p.writes(),2);assert.equal(p.attempts(),2);
 assert.equal(p.listing().title,PD_STOCK_005_R03_RECOVERY_R02.target.title);
 assert.deepEqual(p.listing().tags,[...PD_STOCK_005_R03_RECOVERY_R02.target.tags]);
...
test("unverified ZIP upload fails closed before metadata patch",async()=>{
 process.env.PUBLISH_WRITES_ENABLED="true";process.env.ETSY_B01_WRITE_TOKEN="w";process.env.ETSY_SHOP_ID="23582741";
 const body=exactPdStock005R03RecoveryR02Body("AUTH",PD_STOCK_005_R03_RECOVERY_R02.protectedStateFingerprint);
 const p=provider({persistUpload:false});
 const req=new Request("https://example.test",{method:"POST",headers:{"x-autodigitalpublisher-write-token":"w"}});
 const r=await handlePdStock005R03RecoveryR02(body,hashOperationRequest(body),req,{
  repository:new MemoryOperationLedgerRepository(),fetchImpl:p.fetchImpl,getAccessToken:async()=>"token",
  loadAsset:async()=>Buffer.alloc(62008),verifyAsset:()=>true,now:()=>"2026-09-18T15:45:00.000Z"
 });
 const x=await r.json();
 assert.equal(r.status,202);assert.equal(x.status,"RECONCILIATION_REQUIRED");
 assert.equal(x.ETSY_WRITE_COUNT,0);assert.equal(p.writes(),0);assert.equal(p.attempts(),1);
 assert.equal(p.listing().title,PD_STOCK_005_R03_RECOVERY_R02.current.title);
});
...
test("Recovery post-release QC is fresh read-only and verifies exact final package including Alt Text",async()=>{
 ...
 const req=new Request("https://example.test",{method:"POST",headers:{"x-autodigitalpublisher-write-token":"w"}});
 const release=await handlePdStock005R03RecoveryR02(body,hashOperationRequest(body),req,{
  repository:repo,fetchImpl:p.fetchImpl,getAccessToken:async()=>"token",
  loadAsset:async()=>Buffer.alloc(62008),verifyAsset:()=>true,now:()=>"2026-09-18T15:10:00.000Z"
 });
 assert.equal(release.status,200);
 const writesBefore=p.writes();
 const qc=await verifyPdStock005R03RecoveryR02PostRelease({getAccessToken:async()=>"token",fetchImpl:p.fetchImpl});
...
test("Recovery protected-state verifier fails closed on Alt Text drift with zero writes",async()=>{
 ...
 const r=await verifyPdStock005R03RecoveryR02ProtectedState({getAccessToken:async()=>"token",fetchImpl});
 const x=await r.json();
...
test("Recovery workflow has exact private ZIP staging marker and post-release QC gate",()=>{
 ...
 assert.match(workflow,/PD-STOCK-005-LIVE-CATALOG-V3-R03-RECOVERY-R02-001/);
 ...
 const module=readFileSync("lib/pd-stock-005-r03-recovery-r02.ts","utf8");
 assert.doesNotMatch(module,/method:"DELETE"/);
 assert.match(module,/RECOVER_UPLOAD_ZIP_THEN_PATCH_TITLE_TAGS/);
});

...
import {
  ...
  pdtIpt001R04AuthorizationRequestHash,
...
test("DAY03 compiled listing identity is the canonical frozen R03 fingerprint", () => {
  ...
  assert.equal(
    PDT_IPT_001_R04_DRAFT.gallery.every((asset) => asset.altText.trim().length > 0),
    true
  );
});
...
test("protected-state verification is read-only and checks all seller states", async () => {
  await withEnv(async () => {
    ...
    const response = await verifyPdtIpt001R04DraftProtectedState({
      fetchImpl,
  ...
  await withEnv(async () => {
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      if (url.pathname === `/v3/application/shops/23582741/listings/${listingId}/images` && method === "POST") {
        ...
        assert.equal(altText, PDT_IPT_001_R04_DRAFT.gallery[rank - 1].altText);
        const row = {
    ...
    const psvResponse = await verifyPdtIpt001R04DraftProtectedState(runtime);
    const psv = await psvResponse.json() as { protectedStateFingerprint: string };
    const authorizationId = "PDT-IPT-001-R04-DRAFT-AUTH-20260919-01";
    const body = exactPdtIpt001R04DraftBody(
      authorizationId,
      psv.protectedStateFingerprint,
      COMMIT
    );
    const requestHash = pdtIpt001R04AuthorizationRequestHash(
      authorizationId,
      psv.protectedStateFingerprint,
      COMMIT
    );
    ...
    const response = await handlePdtIpt001R04Draft(
      body,
      requestHash,
      request,
      runtime
...
test("executor fails closed before provider writes when authorized production commit differs", async () => {
  await withEnv(async () => {
    ...
    const body = exactPdtIpt001R04DraftBody(authorizationId, protectedState, wrongCommit);
    const hash = pdtIpt001R04AuthorizationRequestHash(authorizationId, protectedState, wrongCommit);
    const request = new Request("https://example.test/internal", {
      method: "POST",
      headers: { "x-autodigitalpublisher-write-token": TOKEN }
    });
    const response = await handlePdtIpt001R04Draft(body, hash, request, {
      fetchImpl,
      getAccessToken: async () => "token",
      repository: new MemoryOperationLedgerRepository(),
      loadAsset: async () => Buffer.from("test"),

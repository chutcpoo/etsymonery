...
export const ETSY_READBACK_NORMALIZER_VERSION = "1.1.0" as const;
...
function decodeHtmlEntitiesOnce(value: string) {
  return value.replace(/&(?:#(\d+)|#x([0-9a-f]+)|([a-z]+));/gi, (entity, decimal: string | undefined, hex: string | undefined, named: string | undefined) => {
...
function etsyUserText(v: unknown, c: string) {
  return decodeHtmlEntitiesOnce(text(v, c)).normalize("NFC").trim();
}
...
function tags(v:unknown){if(!Array.isArray(v)||!v.every(x=>typeof x==="string"))throw new Error("INVALID_ETSY_READBACK_TAGS");return v.map(x=>etsyUserText(x,"INVALID_ETSY_READBACK_TAG"));}
...
export function normalizeEtsyReadBack(o:EtsyReadBackObservation){
  ...
  return {
    title:etsyUserText(o.title,"INVALID_ETSY_READBACK_TITLE"),
    description:etsyUserText(o.description,"INVALID_ETSY_READBACK_DESCRIPTION"),
    priceUsd:money(o.price),
    tags:tags(o.tags),
    quantity:integer(o.quantity,"INVALID_ETSY_READBACK_QUANTITY"),
    who_made:text(who,"INVALID_ETSY_READBACK_WHO_MADE"),
    when_made:text(when,"INVALID_ETSY_READBACK_WHEN_MADE"),
    taxonomy_id:taxonomy(tax),
    type:text(o.type??"download","INVALID_ETSY_READBACK_TYPE").toLowerCase(),
    state:text(o.state??"draft","INVALID_ETSY_READBACK_STATE").toLowerCase()
  };
}

export function verifyEtsyReadBackIdentity(expectedListingFingerprint:string, observation:EtsyReadBackObservation){
  const expected=expectedListingFingerprint.normalize("NFC").trim().toLowerCase();
  if(!/^[a-f0-9]{64}$/.test(expected))throw new Error("INVALID_EXPECTED_LISTING_FINGERPRINT");
  const normalized=normalizeEtsyReadBack(observation);
  const actual=createListingFingerprint(normalized);
  return Object.freeze({
    normalizerVersion:ETSY_READBACK_NORMALIZER_VERSION,
    status:actual===expected?"MATCH" as const:"IDENTITY_MISMATCH" as const,
    action:actual===expected?"CONTINUE" as const:"STOP" as const,

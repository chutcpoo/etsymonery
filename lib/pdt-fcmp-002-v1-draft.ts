...
import {
  ...
  PDT_FCMP_002_V1_FROZEN_CANDIDATE_FINGERPRINT,
  PDT_FCMP_002_V1_FROZEN_LISTING_FINGERPRINT,
  PDT_FCMP_002_V1_GALLERY,
  PDT_FCMP_002_V1_LISTING_FINGERPRINT,
  PDT_FCMP_002_V1_LISTING_IDENTITY,
  PDT_FCMP_002_V1_OPERATION_ID,
  type PdtFcmpV1Asset
...
const BUYER_FILE_FIELDS = PDT_FCMP_002_V1_BUYER_FILES.map((asset) => ({
...
export type PdtFcmp002V1Runtime = {
  fetchImpl?: typeof fetch;
  getAccessToken?: () => Promise<string>;
  repository?: OperationLedgerRepository;
  now?: () => string;
  verifyAsset?: (file: File, asset: PdtFcmpV1Asset) => Promise<boolean> | boolean;
};
...
function exactPreDraftBaseline(
  ...
  return (
    ...
    current?.listingId === CURRENT_LIVE_LISTING_ID &&
    current?.state === "active" &&
    current?.title === CURRENT_LIVE_LISTING_TITLE
  );
}

async function protectedState(runtime: PdtFcmp002V1Runtime = {}) {
  assertPdtFcmp002V1ManifestFrozen();
  const configuredShopId = Number(process.env.ETSY_SHOP_ID);
  if (configuredShopId !== PDT_FCMP_002_V1_DRAFT.shopId) {
    throw new Error("PDT_FCMP_V1_SHOP_ID_MISMATCH");
  }
  ...
  const state = await getEtsySellerStateSnapshot({
    shopId: PDT_FCMP_002_V1_DRAFT.shopId,
    accessToken,
    dependencies: { fetchImpl }
  });
  const snapshot = {
    operationId: PDT_FCMP_002_V1_OPERATION_ID,
    candidateId: PDT_FCMP_002_V1_DRAFT.candidateId,
  ...
  return {
    ...
    baselineMatches: exactPreDraftBaseline(state)
  };
}

export function pdtFcmp002V1Plan() {
  assertPdtFcmp002V1ManifestFrozen();
  return {
    status: "PLAN_ONLY",
    mode: "POST_RESET_V2_DRAFT_ONLY",
    productId: PDT_FCMP_002_V1_DRAFT.productId,
    productVersion: PDT_FCMP_002_V1_DRAFT.productVersion,
    operationId: PDT_FCMP_002_V1_OPERATION_ID,
    candidateId: PDT_FCMP_002_V1_DRAFT.candidateId,
    candidateFingerprint: PDT_FCMP_002_V1_CANDIDATE_FINGERPRINT,
    listingFingerprint: PDT_FCMP_002_V1_LISTING_FINGERPRINT,
    priceUsd: PDT_FCMP_002_V1_LISTING_IDENTITY.priceUsd,
    imageCount: PDT_FCMP_002_V1_GALLERY.length,
    buyerFileCount: PDT_FCMP_002_V1_BUYER_FILES.length,
...
export async function verifyPdtFcmp002V1ProtectedState(
  runtime: PdtFcmp002V1Runtime = {}
) {
  try {
...
export function exactPdtFcmp002V1AuthorizationContract(
  protectedStateFingerprint: string,
  productionCommit: string,
  deploymentCommit: string
) {
  return {
    authorizationId: PDT_FCMP_002_V1_DRAFT.authorizationId,
    ...
    priceUsd: PDT_FCMP_002_V1_LISTING_IDENTITY.priceUsd,
    gallery: PDT_FCMP_002_V1_GALLERY.map((asset) => ({
      rank: asset.rank,
    ...
    buyerFiles: PDT_FCMP_002_V1_BUYER_FILES.map((asset) => ({
      rank: asset.rank,
      fileName: asset.fileName,
      sizeBytes: asset.sizeBytes,
      sha256: asset.sha256,
...
export function pdtFcmp002V1AuthorizationRequestHash(
  protectedStateFingerprint: string,
  productionCommit: string,
  deploymentCommit: string
) {
  return hashOperationRequest(
    exactPdtFcmp002V1AuthorizationContract(
      protectedStateFingerprint,
      productionCommit,
      deploymentCommit
    )
...
async function verifyAssetFile(
  file: File,
  asset: PdtFcmpV1Asset,
  runtime: PdtFcmp002V1Runtime
) {
  if (file.name.normalize("NFC").trim() !== asset.fileName) {
    throw new Error("PDT_FCMP_V1_ASSET_FILENAME_MISMATCH_" + String(asset.rank));
  }
  if (file.size !== asset.sizeBytes) {
    throw new Error("PDT_FCMP_V1_ASSET_SIZE_MISMATCH_" + String(asset.rank));
  }
  if (runtime.verifyAsset) {
    if (!(await runtime.verifyAsset(file, asset))) {
      throw new Error("PDT_FCMP_V1_ASSET_SHA256_MISMATCH_" + String(asset.rank));
    }
    return;
  }
...
async function parseExactAssets(
  ...
  for (const key of form.keys()) {
    if (!ALLOWED_FORM_FIELDS.has(key)) {
      throw new Error("PDT_FCMP_V1_UNEXPECTED_FORM_FIELD_" + key);
    }
  }
  ...
  for (const item of ASSET_FIELDS) {
    ...
    if (!(value instanceof File)) {
      throw new Error("PDT_FCMP_V1_MISSING_ASSET_" + item.field);
    }
    await verifyAssetFile(value, item.asset, runtime);
    files.set(item.asset.sha256, value);
  }
  if (files.size !== ASSET_FIELDS.length) throw new Error("PDT_FCMP_V1_ASSET_COUNT_MISMATCH");
  return files;
}
...
async function fetchListing(
  ...
  if (!response.ok) {
    throw new Error("PDT_FCMP_V1_LISTING_READBACK_HTTP_" + String(response.status));
  }
  const value = await parseJson(response);
  if (!isRec(value)) throw new Error("PDT_FCMP_V1_LISTING_READBACK_INVALID");
  return value;
}

class PdtFcmp002V1DraftProvider implements ReconciledWriteProvider {
  constructor(
  ...
  private async exactDraftMatches() {
    const response = await this.fetchImpl(
      "https://api.etsy.com/v3/application/shops/" +
        String(PDT_FCMP_002_V1_DRAFT.shopId) +
    ...
    if (!response.ok || !list) {
      throw new Error("PDT_FCMP_V1_DRAFT_LIST_READ_FAILED");
    }
  ...
  async apply(): Promise<ProviderReceipt> {
    if ((await this.exactDraftMatches()).length !== 0) {
      throw new Error("PDT_FCMP_V1_DRAFT_COLLISION");
    }
    const listing = PDT_FCMP_002_V1_LISTING_IDENTITY;
    ...
    if (!response.ok) {
      throw new Error("PDT_FCMP_V1_DRAFT_CREATE_REJECTED_" + String(response.status));
    }
    const payload = await parseJson(response);
...
async function uploadAsset(
  ...
  if (result.status === "RECONCILIATION_REQUIRED") {
    throw new Error("PDT_FCMP_V1_ASSET_RECONCILIATION_REQUIRED_" + String(asset.rank));
  }
  const receipt = result.receipt as ProviderReceipt;
  if (!(await provider.hasResource(receipt.providerResourceId))) {
    throw new Error("PDT_FCMP_V1_ASSET_READBACK_MISMATCH_" + String(asset.rank));
  }
  return receipt;
}
...
async function verifyFinalPersistence(
  ...
  if (listing.state !== "draft" || identity.status !== "MATCH") {
    throw new Error("PDT_FCMP_V1_FINAL_LISTING_IDENTITY_MISMATCH");
  }
  ...
  if (!imagesResponse.ok || !images) {
    throw new Error("PDT_FCMP_V1_FINAL_IMAGE_READBACK_FAILED");
  }
  if (!filesResponse.ok || !files) {
    throw new Error("PDT_FCMP_V1_FINAL_FILE_READBACK_FAILED");
  }

  const orderedImages = [...images].sort(
    (a, b) => Number(a.rank) - Number(b.rank)
  );
  if (orderedImages.length !== PDT_FCMP_002_V1_GALLERY.length) {
    throw new Error("PDT_FCMP_V1_FINAL_GALLERY_COUNT_MISMATCH");
  }
  for (let index = 0; index < PDT_FCMP_002_V1_GALLERY.length; index += 1) {
    ...
    if (
      ...
      throw new Error("PDT_FCMP_V1_FINAL_GALLERY_ORDER_MISMATCH_" + String(expected.rank));
    }
    ...
    if (
      ...
      throw new Error("PDT_FCMP_V1_FINAL_GALLERY_DIMENSION_MISMATCH_" + String(expected.rank));
    }
  }
  ...
  if (orderedFiles.length !== PDT_FCMP_002_V1_BUYER_FILES.length) {
    throw new Error("PDT_FCMP_V1_FINAL_BUYER_FILE_COUNT_MISMATCH");
  }
  for (let index = 0; index < PDT_FCMP_002_V1_BUYER_FILES.length; index += 1) {
    const expected = PDT_FCMP_002_V1_BUYER_FILES[index];
    ...
    if (
      ...
      throw new Error("PDT_FCMP_V1_FINAL_BUYER_FILE_MISMATCH_" + String(expected.rank));
    }
  }
  ...
  const liveListing = sellerState.listings.find(
    (item) => item.listingId === CURRENT_LIVE_LISTING_ID
  );
  const draftListing = sellerState.listings.find(
  ...
  if (
    ...
    liveListing?.state !== "active" ||
    liveListing?.title !== CURRENT_LIVE_LISTING_TITLE ||
    draftListing?.state !== "draft"
  ) {
    throw new Error("PDT_FCMP_V1_FINAL_SELLER_STATE_MISMATCH");
  }
  ...
  return {
    ...
    sellerStateCounts: sellerState.counts,
...
export async function handlePdtFcmp002V1Post(
  request: Request,
  runtime: PdtFcmp002V1Runtime = {}
) {
  ...
  if (contentType.includes("application/json")) {
    ...
    if (!isRec(value) || value.action !== "verify_protected_state") {
      return NextResponse.json(
        {
          status: "BLOCKED_FAIL_CLOSED",
          error: "PDT_FCMP_V1_READ_ONLY_ACTION_REQUIRED",
          ETSY_WRITE_COUNT: 0
        },
        { status: 400 }
      );
    }
    return verifyPdtFcmp002V1ProtectedState(runtime);
  }
  ...
  if (!writesEnabled()) {
    return NextResponse.json(
      {
        status: "BLOCKED_FAIL_CLOSED",
        error: "PDT_FCMP_V1_WRITES_DISABLED",
        ETSY_WRITE_COUNT: 0
      },
      { status: 403 }
    );
  ...
  if (!productionRuntime()) {
    return NextResponse.json(
      {
        status: "BLOCKED_FAIL_CLOSED",
        error: "PDT_FCMP_V1_PRODUCTION_RUNTIME_REQUIRED",
        ETSY_WRITE_COUNT: 0
      },
      { status: 403 }
    );
  ...
  try {
    ...
    const authorizationText = normalizedText(
      ...
      "PDT_FCMP_V1_AUTHORIZATION_TEXT_MISSING"
    );
    if (!secureEqual(authorizationText, PDT_FCMP_002_V1_AUTHORIZATION_TEXT)) {
      throw new Error("PDT_FCMP_V1_AUTHORIZATION_TEXT_INVALID");
    }
    const candidateFingerprint = normalizedText(
      form.get("candidateFingerprint"),
      "PDT_FCMP_V1_CANDIDATE_FINGERPRINT_MISSING"
    ).toLowerCase();
    const listingFingerprint = normalizedText(
      form.get("listingFingerprint"),
      "PDT_FCMP_V1_LISTING_FINGERPRINT_MISSING"
    ).toLowerCase();
    const protectedStateFingerprint = normalizedText(
      form.get("protectedStateFingerprint"),
      "PDT_FCMP_V1_PROTECTED_STATE_FINGERPRINT_MISSING"
    ).toLowerCase();
    const productionCommit = normalizedText(
      form.get("productionCommit"),
      "PDT_FCMP_V1_PRODUCTION_COMMIT_MISSING"
    ).toLowerCase();
    const deploymentCommit = normalizedText(
      form.get("deploymentCommit"),
      "PDT_FCMP_V1_DEPLOYMENT_COMMIT_MISSING"
    ).toLowerCase();
    const suppliedRequestHash = normalizedText(
      form.get("requestHash"),
      "PDT_FCMP_V1_REQUEST_HASH_MISSING"
    ).toLowerCase();

    if (
      candidateFingerprint !== PDT_FCMP_002_V1_FROZEN_CANDIDATE_FINGERPRINT ||
      candidateFingerprint !== PDT_FCMP_002_V1_CANDIDATE_FINGERPRINT
    ) {
      throw new Error("PDT_FCMP_V1_CANDIDATE_FINGERPRINT_MISMATCH");
    }
    if (
      listingFingerprint !== PDT_FCMP_002_V1_FROZEN_LISTING_FINGERPRINT ||
      listingFingerprint !== PDT_FCMP_002_V1_LISTING_FINGERPRINT
    ) {
      throw new Error("PDT_FCMP_V1_LISTING_FINGERPRINT_MISMATCH");
    }
    if (!SHA256.test(protectedStateFingerprint)) {
      throw new Error("PDT_FCMP_V1_PROTECTED_STATE_FINGERPRINT_INVALID");
    }
    if (!SHA256.test(suppliedRequestHash)) {
      throw new Error("PDT_FCMP_V1_REQUEST_HASH_INVALID");
    }
    ...
    if (
      ...
      throw new Error("PDT_FCMP_V1_GIT_VERCEL_COMMIT_MISMATCH");
    }

    const expectedRequestHash = pdtFcmp002V1AuthorizationRequestHash(
      protectedStateFingerprint,
      productionCommit,
      deploymentCommit
    );
    if (!secureEqual(suppliedRequestHash, expectedRequestHash)) {
      throw new Error("PDT_FCMP_V1_AUTHORIZATION_REQUEST_HASH_MISMATCH");
    }

    const files = await parseExactAssets(form, runtime);

    const existing = await repository.load(PDT_FCMP_002_V1_OPERATION_ID);
    if (existing?.status === "SUCCEEDED") {
      if (!secureEqual(existing.requestHash, suppliedRequestHash)) {
        throw new Error("PDT_FCMP_V1_OPERATION_ID_PAYLOAD_MISMATCH");
      }
      return replayResponse(existing);
    }
    if (existing) {
      throw new Error("PDT_FCMP_V1_OPERATION_ALREADY_CLAIMED_DO_NOT_RETRY");
    }
    ...
    if (!state.baselineMatches) {
      throw new Error("PDT_FCMP_V1_PROTECTED_STATE_MISMATCH");
    }
    if (!secureEqual(state.fingerprint, protectedStateFingerprint)) {
      throw new Error("PDT_FCMP_V1_PROTECTED_STATE_DRIFT");
    }

    const contract = exactPdtFcmp002V1AuthorizationContract(
      protectedStateFingerprint,
      productionCommit,
      deploymentCommit
    );
    ...
    if (!secureEqual(parentRequestHash, suppliedRequestHash)) {
      throw new Error("PDT_FCMP_V1_LEDGER_REQUEST_HASH_MISMATCH");
    }

    const token = state.accessToken;
    const draftProvider = new PdtFcmp002V1DraftProvider(token, countedFetch);
    const draftResult = await executeReconciledWrite(repository, draftProvider, {
      operationId: PDT_FCMP_002_V1_OPERATION_ID + ":CREATE_DRAFT",
      kind: "CREATE_DRAFT",
      payload: {
        candidateId: PDT_FCMP_002_V1_DRAFT.candidateId,
        candidateFingerprint: PDT_FCMP_002_V1_CANDIDATE_FINGERPRINT,
    ...
    if (draftResult.status === "RECONCILIATION_REQUIRED") {
      throw new Error("PDT_FCMP_V1_DRAFT_RECONCILIATION_REQUIRED");
    }
    ...
    if (!Number.isSafeInteger(draftListingId) || draftListingId <= 0) {
      throw new Error("PDT_FCMP_V1_DRAFT_ID_INVALID");
    }

    const assetReceipts: ProviderReceipt[] = [];
    for (const asset of [...PDT_FCMP_002_V1_GALLERY, ...PDT_FCMP_002_V1_BUYER_FILES]) {
      const file = files.get(asset.sha256);
      if (!file) throw new Error("PDT_FCMP_V1_STAGED_ASSET_MISSING");
      assetReceipts.push(
        await uploadAsset(
          asset,
          file,

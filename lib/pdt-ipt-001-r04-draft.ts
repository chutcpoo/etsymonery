...
const AUTHORIZATION_ID = /^PDT-IPT-001-R04-DRAFT-AUTH-20260919-[0-9]{2}$/;
...
export const PDT_IPT_001_R04_DRAFT = Object.freeze({
  ...
  candidateId: "ETSY-LISTING-CANDIDATE-PDT-IPT-001-V1-20260919-R04",
  candidateFingerprint: "37c0dab257ad2e90f456ddedefd7a22df5fa922db1df510b2d8581d7079a1562",
  listingFingerprint: "0659c764fa3fe6f0372399c0b2dd38ce363bc5ebb91b75bb23311f87a41f2055",
  title: "Invoice & Payment Tracker Spreadsheet | Excel Small Business Invoice Tracker with Payments, Aging and Dashboard",
...
export const PDT_IPT_001_R04_DRAFT_ASSETS = Object.freeze([
  ...GALLERY,
  BUYER_FILE,
  VIDEO
...
export const PDT_IPT_001_R04_LISTING_FINGERPRINT =
...
function coreDraftMatches(value: Rec) {
  ...
  return (
    value.title === PDT_IPT_001_R04_DRAFT.title &&
    value.description === PDT_IPT_001_R04_DRAFT.description &&
    Math.abs(price - PDT_IPT_001_R04_DRAFT.priceUsd) <= 0.005 &&
    Number(value.quantity) === PDT_IPT_001_R04_DRAFT.quantity &&
    value.who_made === PDT_IPT_001_R04_DRAFT.whoMade &&
    value.when_made === PDT_IPT_001_R04_DRAFT.whenMade &&
    Number(value.taxonomy_id) === PDT_IPT_001_R04_DRAFT.taxonomyId &&
...
async function protectedState(runtime: Runtime = {}) {
  ...
  const collisions = state.listings.filter((item) =>
    item.title?.normalize("NFC").trim() === PDT_IPT_001_R04_DRAFT.title ||
    item.skus.some((sku) => sku.normalize("NFC").trim() === PDT_IPT_001_R04_DRAFT.sku)
  );
  const snapshot = {
    ...
    listingFingerprint: PDT_IPT_001_R04_DRAFT.listingFingerprint,
...
export async function verifyPdtIpt001R04DraftProtectedState(runtime: Runtime = {}) {
  try {
...
export function exactPdtIpt001R04DraftBody(
  authorizationId: string,
  protectedStateFingerprint: string,
  productionCommit: string
) {
  return {
    operation: PDT_IPT_001_R04_DRAFT.operation,
    operationId: PDT_IPT_001_R04_DRAFT.operationId,
    ...
    candidateFingerprint: PDT_IPT_001_R04_DRAFT.candidateFingerprint,
    listingFingerprint: PDT_IPT_001_R04_DRAFT.listingFingerprint,
    scope: PDT_IPT_001_R04_DRAFT.operation,
    sku: PDT_IPT_001_R04_DRAFT.sku,
    isSupply: false,
    shouldAutoRenew: true,
    createStrategy: "MINIMAL_REQUIRED_THEN_SETTINGS_PATCH",
    gallerySha256: PDT_IPT_001_R04_DRAFT.gallery.map((asset) => asset.sha256),
    galleryAltText: PDT_IPT_001_R04_DRAFT.gallery.map((asset) => asset.altText),
    buyerFileSha256: PDT_IPT_001_R04_DRAFT.buyerFile.sha256,
    videoSha256: PDT_IPT_001_R04_DRAFT.video.sha256,
    publishAuthorized: false
  };
}

export function pdtIpt001R04AuthorizationRequestHash(
  authorizationId: string,
  protectedStateFingerprint: string,
  productionCommit: string
) {
...
class R03DraftProvider implements ReconciledWriteProvider {
  ...
  private async exactDraftMatches() {
    const response = await this.fetchImpl(
      `https://api.etsy.com/v3/application/shops/${PDT_IPT_001_R04_DRAFT.shopId}/listings?state=draft&limit=100`,
  ...
  async apply(): Promise<ProviderReceipt> {
    ...
    const body = new URLSearchParams({
      ...
      title: PDT_IPT_001_R04_DRAFT.title,
      description: PDT_IPT_001_R04_DRAFT.description,
      price: PDT_IPT_001_R04_DRAFT.priceUsd.toFixed(2),
      who_made: PDT_IPT_001_R04_DRAFT.whoMade,
      when_made: PDT_IPT_001_R04_DRAFT.whenMade,
      taxonomy_id: String(PDT_IPT_001_R04_DRAFT.taxonomyId),
      is_supply: "false",
      type: "download"
    });
    ...
    try {
      response = await this.fetchImpl(
        `https://api.etsy.com/v3/application/shops/${PDT_IPT_001_R04_DRAFT.shopId}/listings`,
        {
          method: "POST",
          headers: {
...
async function applyOptionalListingSettings(
  ...
  const operationId = `${PDT_IPT_001_R04_DRAFT.operationId}:SETTINGS`;
  const payload = {
    draftListingId,
    candidateFingerprint: PDT_IPT_001_R04_DRAFT.candidateFingerprint,
    tags: [...PDT_IPT_001_R04_DRAFT.tags],
  ...
  try {
    response = await fetchImpl(
      `https://api.etsy.com/v3/application/shops/${PDT_IPT_001_R04_DRAFT.shopId}/listings/${draftListingId}`,
      {
        method: "PATCH",
        headers: {
  ...
  return (await recordOperationResult(
    ...
    now,
    {
      receipt: {
        ...
        shouldAutoRenew: true,
        tagCount: PDT_IPT_001_R04_DRAFT.tags.length
      }
    }
  )).receipt ?? {};
}
...
async function preloadAssets(runtime: Runtime) {
  const load = runtime.loadAsset ??
    (async (asset: Asset | VideoAsset) =>
      loadAuthorizedAsset(PDT_IPT_001_R04_DRAFT.operationId, asset.sha256));
  const loaded = new Map<string, Buffer>();
  for (const asset of PDT_IPT_001_R04_DRAFT_ASSETS) {
...
function childOperationId(asset: Asset) {
  return `${PDT_IPT_001_R04_DRAFT.operationId}:${asset.kind === "UPLOAD_IMAGE" ? "IMAGE" : "FILE"}:${String(asset.rank).padStart(2, "0")}`;
}
...
async function uploadAsset(
  ...
  const payload: EtsyDraftAssetPayload = {
    ...
    candidateId: PDT_IPT_001_R04_DRAFT.candidateId,
    candidateFingerprint: PDT_IPT_001_R04_DRAFT.candidateFingerprint,
    expectedListingFingerprint: PDT_IPT_001_R04_DRAFT.listingFingerprint,
    shopId: Number(PDT_IPT_001_R04_DRAFT.shopId),
    draftListingId,
...
async function ensureSku(
  ...
  const operationId = `${PDT_IPT_001_R04_DRAFT.operationId}:SKU`;
  const payload = {
    draftListingId,
    sku: PDT_IPT_001_R04_DRAFT.sku,
    candidateFingerprint: PDT_IPT_001_R04_DRAFT.candidateFingerprint
  ...
  if (afterProducts.length !== 1 || afterProducts[0].sku !== PDT_IPT_001_R04_DRAFT.sku) {
    await recordOperationResult(
      repository,
      operationId,
      begun.record.requestHash,
  ...
  return (await recordOperationResult(
    ...
    "SUCCEEDED",
    now,
    { receipt: { sku: PDT_IPT_001_R04_DRAFT.sku } }
  )).receipt ?? {};
}
...
async function uploadVideo(
  ...
  const operationId = `${PDT_IPT_001_R04_DRAFT.operationId}:VIDEO:01`;
  const payload = {
    draftListingId,
    candidateFingerprint: PDT_IPT_001_R04_DRAFT.candidateFingerprint,
  ...
  try {
    response = await fetchImpl(
      `https://api.etsy.com/v3/application/shops/${PDT_IPT_001_R04_DRAFT.shopId}/listings/${draftListingId}/videos`,
      {
        method: "POST",
...
async function verifyFinal(
  ...
  const [imagesResponse, filesResponse, inventory, videos] = await Promise.all([
    ...
    fetchImpl(
      `https://api.etsy.com/v3/application/shops/${PDT_IPT_001_R04_DRAFT.shopId}/listings/${draftListingId}/files`,
      { method: "GET", headers: etsyApiHeaders(token), cache: "no-store" }
    ),
...
export async function handlePdtIpt001R04Draft(
  body: ReturnType<typeof exactPdtIpt001R04DraftBody>,
  ...
  try {
    ...
    const existing = await repository.load(PDT_IPT_001_R04_DRAFT.operationId);
    ...
    const parent = await beginOperation(
      repository,
      PDT_IPT_001_R04_DRAFT.operationId,
      body,
      now
    );
    ...
    try {
      ...
      const draftResult = await executeReconciledWrite(repository, draftProvider, {
        operationId: `${PDT_IPT_001_R04_DRAFT.operationId}:CREATE_DRAFT`,
        kind: "CREATE_DRAFT",
        payload: {
          candidateId: PDT_IPT_001_R04_DRAFT.candidateId,
          candidateFingerprint: PDT_IPT_001_R04_DRAFT.candidateFingerprint,
      ...
      for (const asset of PDT_IPT_001_R04_DRAFT_ASSETS) {
        try {
          await (runtime.clearAsset
            ...
            : clearAuthorizedAsset(PDT_IPT_001_R04_DRAFT.operationId, asset.sha256));
        } catch {

...
export const PD_CLEAN_004_VISUAL_R02_RELEASE = Object.freeze({
  ...
  protectedStateDriveId: "113QJf2mSyLMOytsCxcunpPWcEG-e1IDL",
  gallery: [
    {
      order: 1,
...
type GalleryAsset = (typeof PD_CLEAN_004_VISUAL_R02_RELEASE.gallery)[number];
type ReleaseAsset = GalleryAsset | typeof PD_CLEAN_004_VISUAL_R02_RELEASE.video;
...
async function readState(accessToken: string, fetchImpl: typeof fetch): Promise<State> {
  ...
  const shop = PD_CLEAN_004_VISUAL_R02_RELEASE.shopId;
  const listing = PD_CLEAN_004_VISUAL_R02_RELEASE.listingId;
  const urls = {
  ...
  if (!by.listing.response.ok || !isRec(by.listing.value) || !by.images.response.ok || !by.attributes.response.ok || !by.files.response.ok || !by.videos.response.ok || !images || !attributes || !files || !videos) {
    throw new Error("PD_CLEAN_004_R02_READBACK_FAILED");
  }
...
export function exactPdClean004AuthorizationHash(protectedStateFingerprint: string) {
  return hashOperationRequest({
    authorizationId: PD_CLEAN_004_VISUAL_R02_RELEASE.authorizationId,
    operationId: PD_CLEAN_004_VISUAL_R02_RELEASE.operationId,
    operationFingerprint: PD_CLEAN_004_VISUAL_R02_RELEASE.operationFingerprint,
    candidateFingerprint: PD_CLEAN_004_VISUAL_R02_RELEASE.candidateFingerprint,
    scope: PD_CLEAN_004_VISUAL_R02_RELEASE.operation,
    protectedStateFingerprint: protectedStateFingerprint.trim().toLowerCase()
  });
}

export function exactPdClean004ReleaseBody(protectedStateFingerprint: string) {
  return {
    operation: PD_CLEAN_004_VISUAL_R02_RELEASE.operation,
    operationId: PD_CLEAN_004_VISUAL_R02_RELEASE.operationId,
    operationFingerprint: PD_CLEAN_004_VISUAL_R02_RELEASE.operationFingerprint,
    authorizationId: PD_CLEAN_004_VISUAL_R02_RELEASE.authorizationId,
    productId: PD_CLEAN_004_VISUAL_R02_RELEASE.productId,
    productVersion: PD_CLEAN_004_VISUAL_R02_RELEASE.productVersion,
    shopId: PD_CLEAN_004_VISUAL_R02_RELEASE.shopId,
    listingId: PD_CLEAN_004_VISUAL_R02_RELEASE.listingId,
    candidateId: PD_CLEAN_004_VISUAL_R02_RELEASE.candidateId,
    candidateFingerprint: PD_CLEAN_004_VISUAL_R02_RELEASE.candidateFingerprint,
    buildId: PD_CLEAN_004_VISUAL_R02_RELEASE.buildId,
    acceptanceCriteriaId: PD_CLEAN_004_VISUAL_R02_RELEASE.acceptanceCriteriaId,
    protectedStateFingerprint: protectedStateFingerprint.trim().toLowerCase(),
    gallery: PD_CLEAN_004_VISUAL_R02_RELEASE.gallery.map((asset) => ({ ...asset })),
    video: { ...PD_CLEAN_004_VISUAL_R02_RELEASE.video }
  };
}

function writeAuthorizationError(request: Request) {
  if (process.env.PUBLISH_WRITES_ENABLED !== "true") return NextResponse.json({ error: "PD_CLEAN_004_R02_WRITES_DISABLED", ETSY_WRITE_COUNT: 0 }, { status: 403 });
  const expected = process.env.ETSY_B01_WRITE_TOKEN?.trim() ?? "";
  const supplied = request.headers.get(WRITE_HEADER)?.trim() ?? "";
  if (!expected) return NextResponse.json({ error: "PD_CLEAN_004_R02_WRITE_AUTH_NOT_CONFIGURED", ETSY_WRITE_COUNT: 0 }, { status: 503 });
  if (!supplied || !secureEqual(expected, supplied)) return NextResponse.json({ error: "PD_CLEAN_004_R02_UNAUTHORIZED", ETSY_WRITE_COUNT: 0 }, { status: 401 });
  if ((process.env.ETSY_SHOP_ID?.trim() ?? "") !== PD_CLEAN_004_VISUAL_R02_RELEASE.shopId) return NextResponse.json({ error: "PD_CLEAN_004_R02_SHOP_MISMATCH", ETSY_WRITE_COUNT: 0 }, { status: 409 });
  return null;
}
...
function stagingAuthorizationError(request: Request) {
  const expected = process.env.ETSY_B01_WRITE_TOKEN?.trim() ?? "";
  const supplied = request.headers.get(WRITE_HEADER)?.trim() ?? "";
  if (!expected) return NextResponse.json({ error: "PD_CLEAN_004_R02_STAGE_AUTH_NOT_CONFIGURED", ETSY_WRITE_COUNT: 0 }, { status: 503 });
  if (!supplied || !secureEqual(expected, supplied)) return NextResponse.json({ error: "PD_CLEAN_004_R02_STAGE_UNAUTHORIZED", ETSY_WRITE_COUNT: 0 }, { status: 401 });
  return null;
}
...
async function loadAndVerifyAllAssets(runtime: PdClean004VisualR02Runtime) {
  const all: ReleaseAsset[] = [...PD_CLEAN_004_VISUAL_R02_RELEASE.gallery, PD_CLEAN_004_VISUAL_R02_RELEASE.video];
  const loaded: Array<{ asset: ReleaseAsset; bytes: Buffer }> = [];
  for (const asset of all) {
    let bytes: Buffer;
    try {
      bytes = await (runtime.loadAsset ? runtime.loadAsset(asset) : loadAuthorizedAsset(PD_CLEAN_004_VISUAL_R02_RELEASE.operationId, asset.sha256));
    } catch {
      throw new Error(`PD_CLEAN_004_R02_ASSET_NOT_READY_${asset.sha256.slice(0, 8)}`);
    }
    if (!(runtime.verifyAsset ?? defaultVerifyAsset)(bytes, asset)) throw new Error(`PD_CLEAN_004_R02_ASSET_IDENTITY_MISMATCH_${asset.sha256.slice(0, 8)}`);
...
function finalGalleryMatches(state: State, uploadedIds: readonly string[]) {
  if (!protectedCoreMatches(state) || state.images.length !== PD_CLEAN_004_VISUAL_R02_RELEASE.gallery.length) return false;
  const images = [...state.images].sort((a, b) => Number(a.rank) - Number(b.rank));
  return PD_CLEAN_004_VISUAL_R02_RELEASE.gallery.every((asset, index) => {
...
export async function verifyPdClean004R02ProtectedState(runtime: PdClean004VisualR02Runtime = {}) {
  try {
    ...
    return NextResponse.json({ error: "PD_CLEAN_004_R02_PROTECTED_STATE_READ_FAILED", ETSY_WRITE_COUNT: 0 }, { status: 502 });
  }
}

export async function handlePdClean004R02AssetChunk(body: Rec, request: Request) {
  ...
  if (operationId !== PD_CLEAN_004_VISUAL_R02_RELEASE.operationId || !registered) return NextResponse.json({ error: "PD_CLEAN_004_R02_ASSET_NOT_REGISTERED", ETSY_WRITE_COUNT: 0 }, { status: 409 });
...
export async function handlePdClean004R02Release(
  ...
  if (!SHA256.test(protectedSha) || !SHA256.test(authHash)) return NextResponse.json({ error: "PD_CLEAN_004_R02_EXACT_AUTHORIZATION_BINDING_REQUIRED", ETSY_WRITE_COUNT: 0 }, { status: 409 });
  const expectedAuthHash = exactPdClean004AuthorizationHash(protectedSha);
  if (!secureEqual(expectedAuthHash, authHash)) return NextResponse.json({ error: "PD_CLEAN_004_R02_AUTHORIZATION_REQUEST_HASH_MISMATCH", ETSY_WRITE_COUNT: 0 }, { status: 409 });

  const body = exactPdClean004ReleaseBody(protectedSha);
  const repository = runtime.repository ?? new NeonOperationLedgerRepository();
  const exactRequestHash = hashOperationRequest(body);
  const existing = await repository.load(PD_CLEAN_004_VISUAL_R02_RELEASE.operationId);
  if (existing) {
    ...
    return NextResponse.json({ error: "PD_CLEAN_004_R02_ALREADY_CLAIMED_DO_NOT_RETRY", ledgerStatus: existing.status, receipt: existing.receipt, ETSY_WRITE_COUNT: Number(existing.receipt?.ETSY_WRITE_COUNT) || 0 }, { status: existing.status === "RECONCILIATION_REQUIRED" ? 202 : 409 });
  }

  let staged: Awaited<ReturnType<typeof loadAndVerifyAllAssets>>;
  try { staged = await loadAndVerifyAllAssets(runtime); }
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "PD_CLEAN_004_R02_ASSET_ERROR", ETSY_WRITE_COUNT: 0 }, { status: 409 }); }

  const fetchImpl = runtime.fetchImpl ?? fetch;
  const token = await (runtime.getAccessToken ?? getValidEtsyAccessToken)();
  let before: State;
  try { before = await readState(token, fetchImpl); }
  catch { return NextResponse.json({ error: "PD_CLEAN_004_R02_PREREAD_FAILED", ETSY_WRITE_COUNT: 0 }, { status: 502 }); }
  if (!exactPreWriteBaselineMatches(before)) return NextResponse.json({ error: "PD_CLEAN_004_R02_PROTECTED_STATE_MISMATCH", ETSY_WRITE_COUNT: 0 }, { status: 409 });
  const actualProtectedSha = hashOperationRequest(protectedSnapshot(before));
  if (!secureEqual(actualProtectedSha, protectedSha)) return NextResponse.json({ error: "PD_CLEAN_004_R02_FRESH_PSV_FINGERPRINT_MISMATCH", actualProtectedStateFingerprint: actualProtectedSha, ETSY_WRITE_COUNT: 0 }, { status: 409 });

  const now = runtime.now?.() ?? new Date().toISOString();
  const begun = await beginOperation(repository, PD_CLEAN_004_VISUAL_R02_RELEASE.operationId, body, now);
  if (begun.status === "REPLAY") return NextResponse.json({ error: "PD_CLEAN_004_R02_ALREADY_CLAIMED_DO_NOT_RETRY", ETSY_WRITE_COUNT: 0 }, { status: 409 });

  const bySha = new Map(staged.map((entry) => [entry.asset.sha256, entry.bytes]));
  const uploadedIds: string[] = [];
  let confirmedWrites = 0;
  let writeAttempts = 0;
  const imagesUrl = `https://api.etsy.com/v3/application/shops/${PD_CLEAN_004_VISUAL_R02_RELEASE.shopId}/listings/${PD_CLEAN_004_VISUAL_R02_RELEASE.listingId}/images`;

  for (const asset of PD_CLEAN_004_VISUAL_R02_RELEASE.gallery) {
    ...
    try { response = await fetchImpl(imagesUrl, { method: "POST", headers: multipartHeaders(token), body: form, cache: "no-store" }); }
      ...
      return NextResponse.json({ error: "PD_CLEAN_004_R02_GALLERY_UPLOAD_AMBIGUOUS_DO_NOT_RETRY", uploadedIds, ETSY_WRITE_COUNT: confirmedWrites, ETSY_WRITE_ATTEMPT_COUNT: writeAttempts }, { status: 202 });
    }
    ...
    if (!response.ok || !Number.isSafeInteger(imageId) || imageId < 1) {
      ...
      return NextResponse.json({ error: reconciliation ? "PD_CLEAN_004_R02_GALLERY_PARTIAL_DO_NOT_RETRY" : "PD_CLEAN_004_R02_GALLERY_REJECTED", providerStatus: response.status, uploadedIds, ETSY_WRITE_COUNT: confirmedWrites, ETSY_WRITE_ATTEMPT_COUNT: writeAttempts }, { status: reconciliation ? 202 : 502 });
    }
    confirmedWrites += 1;
  ...
  if (!afterGallery || !finalGalleryMatches(afterGallery, uploadedIds) || videoRows(afterGallery).length !== 0) {
    ...
    return NextResponse.json({ error: "PD_CLEAN_004_R02_GALLERY_READBACK_FAILED_DO_NOT_RETRY", uploadedIds, ETSY_WRITE_COUNT: confirmedWrites, ETSY_WRITE_ATTEMPT_COUNT: writeAttempts }, { status: 202 });
  }

  const videoAsset = PD_CLEAN_004_VISUAL_R02_RELEASE.video;
  ...
  const videosUrl = `https://api.etsy.com/v3/application/shops/${PD_CLEAN_004_VISUAL_R02_RELEASE.shopId}/listings/${PD_CLEAN_004_VISUAL_R02_RELEASE.listingId}/videos`;
  ...
  try { videoResponse = await fetchImpl(videosUrl, { method: "POST", headers: multipartHeaders(token), body: videoForm, cache: "no-store" }); }
    ...
    return NextResponse.json({ error: "PD_CLEAN_004_R02_VIDEO_UPLOAD_AMBIGUOUS_DO_NOT_RETRY", uploadedIds, ETSY_WRITE_COUNT: confirmedWrites, ETSY_WRITE_ATTEMPT_COUNT: writeAttempts }, { status: 202 });
  }
  ...
  if (videoResponse.status !== 201 || !Number.isSafeInteger(videoId) || videoId < 1) {
    ...
    return NextResponse.json({ error: "PD_CLEAN_004_R02_VIDEO_UPLOAD_NOT_VERIFIED_DO_NOT_RETRY", providerStatus: videoResponse.status, uploadedIds, ETSY_WRITE_COUNT: confirmedWrites, ETSY_WRITE_ATTEMPT_COUNT: writeAttempts }, { status: 202 });
  }
  confirmedWrites += 1;
  ...
  if (!final || !finalGalleryMatches(final, uploadedIds) || !finalVideo || finalVideo.videoId !== videoId || finalVideo.width !== videoAsset.width || finalVideo.height !== videoAsset.height || !binaryVerified) {
    ...
    return NextResponse.json({ error: "PD_CLEAN_004_R02_FINAL_READBACK_FAILED_DO_NOT_RETRY", uploadedIds, videoId, ETSY_WRITE_COUNT: confirmedWrites, ETSY_WRITE_ATTEMPT_COUNT: writeAttempts }, { status: 202 });
  }
  ...
  for (const asset of [...PD_CLEAN_004_VISUAL_R02_RELEASE.gallery, PD_CLEAN_004_VISUAL_R02_RELEASE.video]) {
    try { await (runtime.clearAsset ? runtime.clearAsset(asset) : clearAuthorizedAsset(PD_CLEAN_004_VISUAL_R02_RELEASE.operationId, asset.sha256)); } catch { /* verified write is complete; cleanup is best effort */ }
  }

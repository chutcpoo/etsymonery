...
export interface OperationLedgerRepository {
  load(operationId: string): Promise<OperationLedgerRecord | null>;
  save(record: OperationLedgerRecord): Promise<void>;
  claim(record: OperationLedgerRecord): Promise<boolean>;
}
...
export class MemoryOperationLedgerRepository implements OperationLedgerRepository{private records=new Map<string,OperationLedgerRecord>();async load(id:string){const r=this.records.get(id);return r?clone(r):null;}async save(r:OperationLedgerRecord){const n=validateOperationLedgerRecord(r);this.records.set(n.operationId,clone(n));}async claim(r:OperationLedgerRecord){const n=validateOperationLedgerRecord(r);if(this.records.has(n.operationId))return false;this.records.set(n.operationId,clone(n));return true;}}
...
export class NeonOperationLedgerRepository implements OperationLedgerRepository{
  ...
  async claim(record:OperationLedgerRecord){await this.ensure();const r=validateOperationLedgerRecord(record),q=sql();const rows=await q`INSERT INTO channel_operation_ledger(operation_id,schema_version,request_hash,attempts,status,receipt,recovery_point,created_at,updated_at) VALUES(${r.operationId},${r.schemaVersion},${r.requestHash},${r.attempts},${r.status},${r.receipt?JSON.stringify(r.receipt):null}::jsonb,${r.recoveryPoint??null},${r.createdAt},${r.updatedAt}) ON CONFLICT(operation_id) DO NOTHING RETURNING operation_id`;return rows.length===1;}
}
export async function beginOperation(repo:OperationLedgerRepository, operationId:string, payload:Record<string,unknown>, now:string){const id=required(operationId,"INVALID_OPERATION_ID"),hash=hashOperationRequest(payload),at=instant(now,"INVALID_OPERATION_TIME"),record=validateOperationLedgerRecord({schemaVersion:OPERATION_LEDGER_SCHEMA_VERSION,operationId:id,requestHash:hash,attempts:1,status:"PENDING",createdAt:at,updatedAt:at});if(await repo.claim(record))return {status:"STARTED" as const,record};const existing=await repo.load(id);if(!existing)throw new Error("OPERATION_CLAIM_NOT_FOUND");if(existing.requestHash!==hash)throw new Error("OPERATION_ID_PAYLOAD_MISMATCH");return {status:"REPLAY" as const,record:existing};}
